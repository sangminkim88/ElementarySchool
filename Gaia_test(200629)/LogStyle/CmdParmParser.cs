...
namespace LogStyle
{
    public class CmdParmParser
    {
        ...
        /// <summary>
        /// Dictionary의 키,값을 "키=값" 그리고 "&"로 연결해줍니다
        /// </summary>
        /// <param name="strDic">Dictionary의 키,값</param>
        /// <returns></returns>
        public static string GetLabelfromDictionary(Dictionary<string, string> strDic)
        {
            List<string> joinStr = new List<string>();             
            foreach(KeyValuePair<string, string> str in strDic)
            {
                string v = str.Key + "=" + str.Value;
                joinStr.Add(v);                
            }            
